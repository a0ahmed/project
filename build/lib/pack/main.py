...
def work_search(query, field_id):
    ...
    if str(field_id) in FieldInfo().field_id:
        ...
        field_parameters = [f'fields%2F{i}' for i in SibInfo(field_id).field_id] + [f'fields%2F{field_id}']
        ...
        if res["meta"]["count"] >= 15:
            ...
            cai_15 = dict(sorted(num_cai.items(), key = lambda item: item[1], reverse = True)[:15])
            ...
            for i in range(len(res['results'])):
                ...
                if res['results'][i]['primary_location']['source'] is None: 
                    ...
                    temp = source_count.append(res['results'][i]['primary_location']['source']['display_name'])

            source_count = dict(Counter(source_count))
            source_15 = dict(sorted(source_count.items(), key = lambda item: item[1], reverse = True)[:15]) # sort dictionary by keys in descending order and return top 15

            publisher = np.array(list(source_15.keys())).reshape((3,5))
            publisher_count = np.array(list(source_15.values())).reshape((3,5))

            return  Heatmap(publisher_count, ta, ta_count, ta_url, Ins_count, source_15, cai_15, publisher, Institutions)
